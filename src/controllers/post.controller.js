...
const updatePost = async (req, res, next) => {
  try {
    const post = await postService.updatePost(req.body, req.params);
    res.status(200).json({ data: post });
  } catch (err) {
    next(err);
  }
...
const deletePost = async (req, res, next) => {
  try {
    const post = await postService.deletePost(req.user, req.params);
...
const findLatestComments = async (req, res, next) => {
  try {
    const comments = await postService.findLatestComments(
      req.params,
      req.query
    );
    res.status(200).json({ data: comments });
